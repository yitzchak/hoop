...
(defmethod expand (var (action (eql :in)) initform &rest initargs &key by &allow-other-keys)
  (declare (ignore initargs))
  (make-instance 'in-clause :var var :initform initform :by (or by #'cdr)))

(defmethod expand (var (action (eql :on)) initform &rest initargs &key by &allow-other-keys)
  (declare (ignore initargs))
...
(defmethod wrap-inner ((clause in-clause) form)
  `(symbol-macrolet ((,(var clause) (car ,(list-var clause))))
     ,form))

(defmethod wrap-inner ((clause on-clause) form)
  `(symbol-macrolet ((,(var clause) ,(list-var clause)))
     ,form))
